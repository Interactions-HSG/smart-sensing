...
CY_ISR(I2CM_I2C_ISR)
{
...
#ifdef I2CM_I2C_ISR_ENTRY_CALLBACK
    I2CM_I2C_ISR_EntryCallback();
#endif /* I2CM_I2C_ISR_ENTRY_CALLBACK */

#if (I2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2CM_I2C_ACK_ADDR;
                ...
                if(I2CM_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2CM_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        ...
                        */
                        if(I2CM_CHECK_INTR_RX_MASKED(I2CM_INTR_RX_FULL))
                        {
                            ...
                            if(diffCount > I2CM_I2C_FIFO_SIZE)
                            {
                                diffCount = I2CM_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        ...
                        {
                            while(I2CM_I2C_FIFO_SIZE != I2CM_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2CM_mstrWrBufIndexTmp < I2CM_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(I2CM_mstrWrBufIndexTmp == (I2CM_mstrWrBufSize - 1u))
                                    {
        ...
        #if(I2CM_I2C_SLAVE)
        {
            ...
            */
            if(I2CM_CHECK_INTR_SLAVE_MASKED(I2CM_INTR_SLAVE_I2C_WRITE_STOP))
            {
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2CM_I2C_CTRL_REG & I2CM_I2C_CTRL_S_READY_DATA_ACK))
                {
            ...
            */
        ...
        #endif /* (I2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
            ...
            #endif /* (I2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    I2CM_ENABLE_INTR_SLAVE(I2CM_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CM_CY_SCBIP_V0) */
            ...
            */
            if(I2CM_CHECK_INTR_RX_MASKED(I2CM_INTR_RX_FULL))
            {
                ...
                diffCount =  I2CM_slWrBufSize -
                            (I2CM_slWrBufIndex + I2CM_I2C_FIFO_SIZE);

                if(diffCount > I2CM_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2CM_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                while(I2CM_I2C_FIFO_SIZE != I2CM_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    ...
                    /* Probably Overflow */
                    {
                        ...
                        if(I2CM_slOverFlowCount <= I2CM_I2C_TX_OVERFLOW_COUNT)
                        {
    ...
    */
    ...
    {
        I2CM_CTRL_REG &= (uint32) ~I2CM_CTRL_ENABLED; /* Disable scb IP */

        I2CM_state = I2CM_I2C_FSM_IDLE;

        I2CM_DISABLE_SLAVE_AUTO_DATA;
        I2CM_DISABLE_MASTER_AUTO_DATA;
        ...
        I2CM_CTRL_REG |= (uint32) I2CM_CTRL_ENABLED;  /* Enable scb IP */
    }

#ifdef I2CM_I2C_ISR_EXIT_CALLBACK
    I2CM_I2C_ISR_ExitCallback();
#endif /* I2CM_I2C_ISR_EXIT_CALLBACK */
